function [l_u_pos_i, l_p_pos_i] = l_u_p_pos_update_foursquare( trainset,rank_gamma_p,rank_delta,u_l,p_l,matrix_feature,lamda,i )
%UNTITLED2 Summary of this function goes here
%   Detailed explanation goes here

%提取第user_unique_m(i_test)个用户的gamma
    temp_gamma_old=rank_gamma_p(:,trainset(i==trainset(:,3),16));
    temp_gamma=repmat(temp_gamma_old',[1 1 matrix_feature]);
    %提取第user_unique_m(i_test)个用户的delta
    temp_delta=repmat(rank_delta(:,trainset(i==trainset(:,3),16))',[1 1 matrix_feature]);
    temp_u_l=u_l(trainset(i==trainset(:,3),1),:,:);
    temp_up_l_u=sum(temp_gamma .* temp_delta .* temp_u_l,1);

    temp_p_l=p_l(trainset(i==trainset(:,3),2),:,:);
...
%     temp_u_l_new=temp_up ./ temp_dowm_new;
    ...
    l_u_pos_i=temp_up_l_u ./ temp_dowm_new;
    l_p_pos_i=temp_up_l_p ./ temp_dowm_new;
